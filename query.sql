...
SELECT * FROM projects WHERE user_uuid = ? AND archived_at IS NOT NULL;
...
-- name: RetrieveArchivedProject :one
...
-- name: ListArchivedProjectTasks :many
SELECT pt.*, p.name AS project_name FROM project_tasks pt
JOIN projects p ON p.uuid = pt.project_uuid
...
-- name: RetrieveArchivedProjectTask :one
